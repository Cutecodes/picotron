...
from picotron.data_parallel.zero1_zero2 import ZeroOptimizer
...
def train_step(model, data_loader, device):
    ...
    for i in range(data_loader.grad_acc_steps):
        ...
        # for zero, we don't do gradient synchronization here
...
if __name__ == "__main__":
    ...
    optimizer = ZeroOptimizer(model.parameters(), lr=config["training"]["learning_rate"], partition_grads=True, **extra_args)
    ...
    if is_wandb_rank:
        ...
        print(f"理论节省: ~{(param_count/1e6*10*(1- 1/world_size)) :.2f}M")
        print("="*60 + "\n")

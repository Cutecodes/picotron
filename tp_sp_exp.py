...
from picotron.tensor_parallel.sequence_parallel import apply_sequence_parallel
...
def compute_mem_save(model_config, config):
    ...
    mbs = config["training"]["micro_batch_size"]
    tp_size = config["distributed"]["tp_size"]

    em_save_mem = (4 * mbs * seq_len * hidden_size) * (1- 1/tp_size)

    out_save_mem = (4 * mbs * seq_len * vocab_size) * (1- 1/tp_size)
    
    # no dropout
    decode_mem_per_layer = 64 * mbs * seq_len * hidden_size + 9 * mbs * num_heads * seq_len**2
    decode_mem_per_layer_tp = 8 * mbs * seq_len * hidden_size /tp_size + (56 * mbs * seq_len * hidden_size + 9 * mbs * num_heads * seq_len**2) / tp_size
    decode_save_mem = num_layers * (decode_mem_per_layer-decode_mem_per_layer_tp)

    return (em_save_mem + out_save_mem + decode_save_mem)/1e6
...
if __name__ == "__main__":
    ...
    assert config["distributed"]["pp_size"] == 1
    ...
    with init_model_with_dematerialized_weights():
        ...
        if pgm.process_group_manager.tp_world_size > 1:
            ...
            model = apply_sequence_parallel(model)
    ...
    if is_wandb_rank:
        print("="*60)
        print(f"张量并行实验 (World Size = {world_size})")
        print("="*60)
        print(f"参数量: {param_count/1e6:.2f}M")
    
    peak_mem = torch.cuda.max_memory_allocated(device) / 1e9
    mem = torch.cuda.memory_allocated(device) / 1e9
    save_mem = compute_mem_save(model_config, config)
    if is_wandb_rank:
        print(f"每个GPU峰值显存: {peak_mem:.3f} GB")
        print(f"持久化显存: {mem:.3f} GB")
        print(f"理论节省: ~:   {save_mem:.3f} MB")
        print("="*60 + "\n")
